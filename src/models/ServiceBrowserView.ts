...
const debug = require('debug')('Ferdium:Models:ServiceBrowserView');
...
export class ServiceBrowserView {
        ...
        webPreferences: {
          ...
          preload:
            recipeId === TODOS_RECIPE_ID
              ? `${__dirname}/../features/todos/preload.js`
              : `${__dirname}/../webview/recipe.js`,
    ...
    if (!this.isRestricted) {
      ...
      if (!this.bounds) {
        this.bounds = {
          x: TAB_BAR_WIDTH,
          y: 0,
          width: bounds.width - TAB_BAR_WIDTH,
  ...
  initialize() {
    if (!this.isRestricted) {
      ...
      if (this.isTodos) {
        ...
        this.window.webContents.send(IPC.TODOS_HOST_CHANNEL, {
          action: 'setWebContentsId',
  ...
  update({
    ...
    config?: Partial<Pick<IServiceConfig, 'name' | 'url'>>;
    state?: Partial<IServiceState>;
  }) {
    ...
    this.webContents.session.setSpellCheckerEnabled(
      this.state.isSpellcheckerEnabled,
    );

    if (isSpellcheckerEnabled) {
      this.webContents.session.setSpellCheckerLanguages([
        spellcheckerLanguage ||
          DEFAULT_APP_SETTINGS_VANILLA.spellcheckerLanguage,
      ]);
    }
  }
  ...
  enableContextMenu() {
    ...
    this.webContents.on('context-menu', async (e, props) => {
      ipcMain.once(
        ...
        (requestLocaleEvent, { locale }) => {
          ...
          debug(
            ...
            this.settings.get('spellcheckerLanguage'),
          );
          ...
          const menu = Menu.buildFromTemplate(
            buildMenuTpl({
              ...
              defaultSpellcheckerLanguage: this.settings.get(
                'spellcheckerLanguage',
              ),
              spellcheckerLanguage,
              onUpdateSpellcheckerLanguage: data => {
                if (data === 'reset') {
                  ...
                  spellcheckerLanguage = this.settings.get(
                    'spellcheckerLanguage',
                  );
                } else {
                ...
                this.window.webContents.send(UPDATE_SPELLCHECKING_LANGUAGE, {
                  serviceId: this.config.id,
                  locale: spellcheckerLanguage,
                });
              },
            }),
          );
      ...
      this.window.webContents.send(REQUEST_SERVICE_SPELLCHECKING_LANGUAGE, {
        serviceId: this.config.id,
      });
    });
  }

  async resize({ width, height, x, y }: Rectangle) {
    const bounds = this.view.getBounds();
    const newBounds = {
      width: Number.parseInt((width ?? bounds.width).toFixed(0), 10),
      height: Number.parseInt((height ?? bounds.height).toFixed(0), 10),
      x: Number.parseInt((x ?? bounds.x).toFixed(0), 10),
      y: Number.parseInt((y ?? bounds.y).toFixed(0), 10),
    };
  ...
  enableKnownCertificateHosts() {
    ...
    this.webContents.session.setCertificateVerifyProc((request, callback) => {
      ...
      if (knownHosts.some(item => item.includes(hostname))) {
        callback(0);
      } else {
        callback(-3);
      }
  ...
  enableOnHeadersReceived() {
    this.webContents.session.webRequest.onHeadersReceived((...args) => {
  ...
  setWebContentsState(state: Partial<IWebContentsState>) {
    ...
    if (!this.window.isDestroyed()) {
      this.window.webContents.send(UPDATE_SERVICE_STATE, {
        serviceId: this.config.id,
        state: this.webContentsState,
      });
    }
  }
